<template lang="">
  <table class="table w-100 pa-3 pb-3">
    <tbody class="table-group-divider">
      ...
      <tr>
        <td scope="row" class="text-start text-table">
          Nước: ({{ store.editPay.qtyMember }} người)
        </td>
        <td></td>
        <td></td>
        <td class="text-end">
...
export default {
  data() {
    return {
      UrlBank:`https://img.vietqr.io/image/STB-060109675711-qr_only.png?amount=${store.editPay.total}&addInfo=Phong${store.editPay.name}` 
    }
  },
};
</script>
